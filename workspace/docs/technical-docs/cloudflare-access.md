...
| --- | --- | --- |
| **Prod** `condat-judo.sebastien-berland.workers.dev` | Aucune (site public : vitrine) | Lien de connexion + session (spec 005a) |
...
Pourquoi aucune identité tirée d'Access : l'app a sa propre authentification (parents sur
...
Navigateur ──▶ Cloudflare Access (edge, preview uniquement) : code PIN par email, refuse l'anonyme (302)
              ...
              Worker condat-judo-preview : ne lit RIEN d'Access
                 ▼
              /api/me ──▶ seam resolveUser ──▶ session de l'app (cookie __Host-session) — 401 sans elle
```
...
- **Verrou ≠ compte** : passer Access ne donne aucun droit dans l'app ; il faut ensuite se
  connecter à l'app avec son lien de connexion (en qualif : `deploy/lien-connexion.ps1 -Cible preview`).
...
- [x] Après code PIN : le site s'affiche ; `/api/me` → « Non connecté » tant qu'on ne s'est pas
  connecté à l'app (lien de connexion, spec 005a)
