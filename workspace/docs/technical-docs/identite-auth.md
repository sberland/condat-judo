...
Requête ──▶ fournisseur d'identité ──▶ Identite { provider, subject, email? }
               ├─ cf-access (runtime) : ctx.access — audience = CF_ACCESS_AUD, puis getIdentity() → user_uuid
               ├─ cf-access (JWT)     : en-tête Cf-Access-Jwt-Assertion vérifié (signature, aud, iss, exp)
...
- **Deux canaux pour Access, un seul `subject`** (constat du 2026-09-23 sur la preview
  `*.workers.dev`, via les journaux du Worker) : Access authentifie en amont mais ne transmet
  **ni l'en-tête `Cf-Access-Jwt-Assertion`, ni le cookie `CF_Authorization`** (aucun cookie
  n'atteint le Worker). L'identité validée est exposée par le runtime Workers via **`ctx.access`**
  (non falsifiable par le client) : on contrôle `ctx.access.aud` = `CF_ACCESS_AUD` (notre
  application) puis `getIdentity()`. Sur un domaine personnalisé, Access injecte l'en-tête : le
  JWT est alors vérifié par `access-jwt.ts`. Dans les deux cas `subject` = identifiant utilisateur
  Access (`user_uuid` = claim `sub`) → même ligne `identites`.
- **Journalisation** (`wrangler tail`) : motifs de refus (audience, signature…) et absence
  d'identité sur `/api/me` ; jamais de jeton ni d'email.
- **Futures routes d'écriture** : l'authentification Access reposant sur un cookie de session du
  navigateur, toute route `POST/PUT/DELETE` devra se protéger du CSRF (contrôle de
